...
parser.add_argument('-outfile',default='./result.npz')
...
if args.scenario=='full':
	predict_result_origin=np.where(np.sort(model.predict(f_evaluate_origin),axis=1) > 0.5, 1, 0)
	predict_result_defense=np.where(np.sort(model.predict(f_evaluate_noise),axis=1) > 0.5, 1, 0)
else:
	raise NotImplementedError
...
np.savez(args.outfile,
	...
	f_evaluate_origin=f_evaluate_origin,
	f_evaluate_noise=f_evaluate_noise,
	f_evaluate_origin_score=f_evaluate_origin_score,
	f_evaluate_defense_score=f_evaluate_defense_score,
	predict_result_origin_logit=model.predict(f_evaluate_origin),
	predict_result_defense_logit=model.predict(f_evaluate_noise),
	predict_result_origin=np.sort(model.predict(f_evaluate_origin),axis=1),
	predict_result_defense=np.sort(model.predict(f_evaluate_noise),axis=1)
)
